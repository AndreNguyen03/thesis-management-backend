...
const lecturers = [
    ...
    },
    {
        ...
        researchInterests: [
            "Các phương pháp thiết kế và phát triển Web"
        ]
    },
    {
    ...
    },
    {
        ...
        areaInterest: [
            "Các kĩ thuật đồ họa nâng cao trong game 3D",
            "các giải pháp server cho game trực tuyến"
        ],
        researchInterests: [
            "Quy trình và công nghệ sản xuất game"
        ]
    },
    {
...
for (const lecturer of lecturers) {
    ...
    const lecturerMatches = new Map<string, {
        concept: any,
        queries: string[],
    ...
    if (lecturerMatches.size > 0) {
        console.log(`\n  🎯 Matched Concepts:`)
        const sortedMatches = Array.from(lecturerMatches.values())
            .sort((a, b) => b.queries.length - a.queries.length)
        
        for (const match of sortedMatches) {
            console.log(`     • ${match.concept.label} (${match.concept.key})`)
            console.log(`       Matched ${match.queries.length} time(s): ${match.queries.slice(0, 2).join(', ')}${match.queries.length > 2 ? '...' : ''}`)
        }
    }
