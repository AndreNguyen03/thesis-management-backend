...
@Injectable()
export class TopicRegisteringSearchTool {
    constructor(
        ...
        private readonly getCurrentPeriod: GetPeriodInfoProvider
    ) {}
...
OUTPUT: Danh sách đề tài kèm thông tin chi tiết (tên, mô tả, yêu cầu, giảng viên)
            ...
            func: async ({ query, limit }) => {
                try {
                    ...
                    // b4: các lấy kỳ của khoa của sinh viên đang mở đăng ký
                    // B5: Populate topic tiêu chuẩn từ DB
                    const topics = await this.getTopicProvider.getStandarStructureTopicsByTopicIds(topicIds, limit)

                    // B6: Format kết quả cho LLM
