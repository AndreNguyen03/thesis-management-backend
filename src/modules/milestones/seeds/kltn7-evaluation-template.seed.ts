...
export const KLTN7_EVALUATION_TEMPLATE: Partial<EvaluationTemplate> = {
    ...
    facultyId: 'CNTT', // Cần thay bằng ObjectId thực tế của Faculty CNTT
    version: 1,
    isActive: true,
...
/**
 * Script để seed template vào database
 * Chạy: npm run seed:evaluation-template
 */
export async function seedKLTN7Template(evaluationTemplateModel: any, facultyId: string, createdBy: string) {
    const existingTemplate = await evaluationTemplateModel.findOne({
        name: KLTN7_EVALUATION_TEMPLATE.name,
        isActive: true
    })

    if (existingTemplate) {
        console.log('✅ KLTN-7 template already exists')
    ...
    console.log('✅ Created KLTN-7 evaluation template:', template._id)
    return template
}
