...
@Injectable()
export class RegistrationPhaseProvider {
    constructor(
    ...
    async getLecturerDashboard(periodId: string, lecturerId: string) {
        ...
        pipeline.push({
            $group: {
                ...
                pendingCount: {
                    $sum: {
                        $cond: [{ $eq: ['$registrations.status', 'pending'] }, 1, 0]
                    }
                },
                approvedCount: {
                    $sum: {
                        $cond: [{ $eq: ['$registrations.status', 'approved'] }, 1, 0]
                    }
                },
                rejectedCount: {
                    $sum: {
                        $cond: [{ $eq: ['$registrations.status', 'rejected'] }, 1, 0]
                    }
                }
            }
        })

        // 4. Chọn các trường trả về
        pipeline.push({
            $project: {
                _id: 1,
                titleVN: 1,
