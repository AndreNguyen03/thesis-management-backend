 ...
 * Giữ nguyên multi-word phrases quan trọng
 */
function tokenize(text) {
    ...
    for (const part of parts) {
        ...
        if (trimmed) {
            ...
            if (words.length > 1) {
                words.forEach((w) => {
                    if (w.length > 2) tokens.add(w) // Skip very short words
                })
            }
        }
    }
    ...
    for (const token of tokens) {
        ...
        for (const [key, synonymList] of Object.entries(SYNONYMS)) {
            if (normalize(key) === token || synonymList.some((syn) => normalize(syn) === token)) {
                // Add the canonical form and all synonyms
...
/**
 * Xử lý array of texts (như areaInterest[], researchInterests[])
 */
function normalizeArray(textArray) {
